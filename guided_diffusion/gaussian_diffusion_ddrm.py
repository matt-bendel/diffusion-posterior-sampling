...
class GaussianDiffusion:
    ...
    def p_sample_loop(self,
        ...
        largest_alphas = torch.tensor([self.alphas_cumprod[-1]]).unsqueeze(1).unsqueeze(1).unsqueeze(1).repeat(x.shape[0], 1, 1, 1)
        largest_alphas = largest_alphas.to(x.device)
        ...
        for idx in pbar:
            ...
            et = self.predict_eps_from_x_start(xt, t, out['pred_xstart'])
            ...
            # missing pixels
            Vt_xt_mod_next = V_t_x0 + sigma_tilde_nextC * svd.Vt(et) + std_nextC * torch.randn_like(V_t_x0)
            ...
            if record:
                if idx % 5 == 0:
                    file_path = f"/storage/matt_models/ddrm/ffhq/x_{str(idx).zfill(4)}.png"
...
class DDIM(SpacedDiffusion):
    ...
    def p_sample(self, model, x, t, y, cond, svd, noise_sig, eta=0.85):
