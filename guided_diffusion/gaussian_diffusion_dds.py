...
def create_sampler(sampler,
    ...
    new_betas = get_named_beta_schedule('poly_4', steps)
...
class DDPM(SpacedDiffusion):
    def denoise(self, model, x, t, y, cond, vamp, noise_sig, truth):
        if not cond:
            ...
            Acg = lambda vec, gamma=gam: vec + gamma * vamp.svd.Ht(vamp.svd.H(vec)).view(vec.shape[0], 3, 256, 256)
            ycg = pred_xstart + gam*vamp.svd.Ht(y).view(y.shape[0], 3, 256, 256)
            pred_xstart = CG(Acg, ycg, pred_xstart.clone(), 5)

        return {'pred_xstart': pred_xstart, 'gamma_1': None, 'gamma_2': None, 'eta_1': None, 'eta_2': None, 'mse_1': None, 'mse_2': None}


def CG(A, b, x, M=5, eps=1e-5):
    r = b - A(x)
    p = r.clone()
    rsold = torch.matmul(r.view(r.shape[0], -1), r.view(r.shape[0], -1).T)

    for i in range(M):
        Ap = A(p)
        a = rsold / torch.matmul(p.view(p.shape[0], -1), Ap.view(p.shape[0], -1).T)
