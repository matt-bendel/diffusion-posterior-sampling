...
class VAMP:
    ...
    def f_1(self, r_1, gamma_1, x_t, y, t_alpha_bar, noise_sig):
        ...
        nonzero_singular_mult = (evals[None, :] + r_sig_inv ** 2 + gamma_1[:, 0, None]) ** -1

        mu_1 = torch.zeros(right_term.shape).to(right_term.device)
        mu_1[:, :evals.shape[0]] = nonzero_singular_mult * right_term[:, :evals.shape[0]]
        if self.Q > 1:
            mu_1[:, evals.shape[0]:] = (r_sig_inv ** 2 + gamma_1[:, 1, None]) ** -1 * right_term[:, evals.shape[0]:]
    ...
    def renoising(self, mu_1, eta_1, gamma_2):
        ...
        zeros = torch.zeros(mu_1.shape[0], 1).to(mu_1.device)
        ...
        v_1_measured = torch.maximum(v_1_measured, zeros)[:, 0]
        mu_1_noised = torch.zeros(mu_1.shape).to(mu_1.device)
        mu_1_noised[:, :singulars.shape[0]] = (mu_1 + noise * v_1_measured.sqrt()[:, None])[:, :singulars.shape[0]]
        if self.Q > 1:
            v_1_nonmeasured = 1 / gamma_2 - 1 / eta_1[:, 1]
            v_1_nonmeasured = torch.maximum(v_1_nonmeasured, zeros)[:, 0]
            mu_1_noised[:, singulars.shape[0]:] = (mu_1 + noise * v_1_nonmeasured.sqrt()[:, None])[:, singulars.shape[0]:]
