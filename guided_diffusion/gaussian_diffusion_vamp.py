...
class DDPM(SpacedDiffusion):
    def denoise(self, model, x, t, y, cond, vamp, noise_sig, truth):
        if not cond:
            ...
            pred_xstart, gamma_1, gamma_2, eta_1, eta_2, _, _, _, _ = vamp.run_vamp_reverse_test(x, y, t, noise_sig=torch.tensor(noise_sig).to(x.device), prob_name='', gt=None, use_damping=True)

        mse_1 = pred_xstart.mean()#((pred_xstart - truth) ** 2).sum() / (3 * 256 * 256)
        mse_2 = pred_xstart.mean()#((pred_xstart - truth) ** 2).sum() / (3 * 256 * 256)
