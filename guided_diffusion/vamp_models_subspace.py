...
class VAMP:
    ...
    def f_1(self, r_1, gamma_1, x_t, y, t_alpha_bar, noise_sig):
        ...
        if self.Q > 1:
            mu_1[:, evals.shape[0]:] = (r_sig_inv ** 2 + gamma_1[:, 1]) * mu_1[:, evals.shape[0]:]
    ...
    def linear_estimation(self, r_1, gamma_1, x_t, y, t_alpha_bar, noise_sig, gt=None):
        ...
        if self.Q > 1:
            r_2[:, singulars.shape[0]:] = ((eta_1[:, 1, None] * mu_1 - gamma_1[:, 1, None] * r_1) / gamma_2[:, 1,None])[:, singulars.shape[0]:]
    ...
    def denoising(self, r_2, gamma_2, t, vamp_iter=0, noise=False, gt=None):
        ...
        print(((eta_2[:, 0, None] * mu_2 - gamma_2[:, 0, None] * r_2) / gamma_1[:, 0, None])[:, :singulars.shape[0]].shape)
        exit()
